...
import msalReact, { IMsalContext, MsalAuthenticationResult } from '@azure/msal-react';
...
describe('Auth component', () => {
    it('Should show an error message when there is an MSAL authentication error.', async () => {
        ...
        jest.spyOn(msalReact, 'useMsalAuthentication').mockReturnValueOnce({ error: {} } as MsalAuthenticationResult);
    ...
    it('Should show an error message when there is a server error.', async () => {
        jest.spyOn(sep, 'useGetUserQuery').mockReturnValueOnce({ error: {}, data: null, refetch: jest.fn() });
    ...
    it('Should show "Loading user..." when the user request is in progress.', async () => {
        jest.spyOn(sep, 'useGetUserQuery').mockReturnValueOnce({ error: null, isLoading: true, data: null, refetch: jest.fn() });
        const { getByText } = renderWithProviders(<Auth />);
    ...
    it('should request the logged-in user data when authenticated', async () => {
        ...
        await waitFor(() => {
            ...
            expect(sep.sepAPI.endpoints.getUser.select('me')(store.getState()).data?.id).toEqual(globals.loggedInUserID)
        });
    });
